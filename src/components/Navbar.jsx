  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50">
      ...
      <div className={`xl:hidden overflow-hidden transition-all duration-400 ${mobileOpen ? 'max-h-[85vh] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="bg-navy-950/98 backdrop-blur-xl border-t border-white/10 max-h-[80vh] overflow-y-auto">
          {/* Mobile trust badges */}
          <div className="px-4 pt-4 pb-3 flex flex-wrap gap-2 justify-center border-b border-white/5">
            {trustItems.map(({ icon: Icon, text }) => (
              <span key={text} className="inline-flex items-center gap-1.5 text-[10px] font-semibold tracking-wider uppercase text-white/40 bg-white/[0.05] border border-white/10 px-2.5 py-1 rounded-full">
                <Icon size={10} className="text-gold-500" />
                {text}
              </span>
            ))}
          </div>
            ...
            {Object.entries(langOptions).map(([code, opt]) => (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${
                  code === lang ? 'bg-gold-500/90 border-gold-400 text-white' : 'bg-white/[0.06] border-white/15 text-white/60 hover:bg-white/10'
                }`}
              >
                ...
                {link.children ? (
                  <>
                    <button
                      ...
                      className={`flex items-center justify-between w-full py-3.5 text-sm font-semibold border-b border-white/[0.06] transition-colors ${
                        isActive(link.href) ? 'text-gold-400' : 'text-white/85'
                      }`}
                    >
                      {link.label}
                    ...
                    {mobileExpanded === link.label && (
                      <div className="pl-3 py-2 space-y-0.5 border-b border-white/[0.04]">
                        <Link to={link.href} className="block py-2 text-xs font-bold text-gold-500 uppercase tracking-wider">
                          {p.viewAll} →
                        </Link>
                        {link.children.map((child) => (
                          <Link key={child.href} to={child.href} className="block py-2 text-sm text-white/60 hover:text-gold-400 transition-colors">
                            {child.label}
                          </Link>
                        ))}
                      </div>
                  ...
                  <Link
                    to={link.href}
                    className={`block py-3.5 text-sm font-semibold border-b border-white/[0.06] transition-colors ${
                      isActive(link.href) ? 'text-gold-400' : 'text-white/85 hover:text-gold-400'
                    }`}
                  >
                    {link.label}
                  </Link>
