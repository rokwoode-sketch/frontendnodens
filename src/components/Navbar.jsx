  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between">
            ...
            >
              {link.href.startsWith('/#') ? (
                <button
                  ...
                  className="flex items-center gap-1 text-white/90 hover:text-gold-400 font-medium text-sm px-3 py-2 rounded-lg transition-colors duration-200"
                >
                  {link.label}
                </button>
              ) : (
        ...
        <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <div className="relative" onMouseEnter={() => setLangOpen(true)} onMouseLeave={() => setLangOpen(false)}>
            ...
            {langOpen && (
              <div className={`absolute top-full ${isRTL ? 'left-0' : 'right-0'} mt-1 bg-white rounded-xl shadow-2xl border border-gray-100 py-1.5 min-w-[140px] z-50`}>
      ...
      <div className={`lg:hidden overflow-hidden transition-all duration-300 ${mobileOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="bg-navy-950 border-t border-white/10 px-4 pb-6 pt-4">
          ...
          <a href="https://wa.me/905546745516" target="_blank" rel="noopener noreferrer"
